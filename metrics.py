...
def calmetric_complex(pred_recon, gt_recon):
    if gt_recon.ndim == 4:
        psnr_array = np.zeros((gt_recon.shape[-2], gt_recon.shape[-1]))
        ssim_array = np.zeros((gt_recon.shape[-2], gt_recon.shape[-1]))
        nmse_array = np.zeros((gt_recon.shape[-2], gt_recon.shape[-1]))
        ...
        for i in range(gt_recon.shape[-2]):
            for j in range(gt_recon.shape[-1]):
                pred, gt = pred_recon[:, :, i, j], gt_recon[:, :, i, j]
                psnr_array[i, j] = 0.5*(psnr(np.real(gt) / np.real(gt).max(), np.real(pred) / np.real(pred).max())+psnr(np.imag(gt) / np.imag(gt).max(), np.imag(pred) / np.imag(pred).max()))
                ssim_array[i, j] = 0.5*(ssim(np.real(gt) / np.real(gt).max(), np.real(pred) / np.real(pred).max())+ssim(np.imag(gt) / np.imag(gt).max(), np.imag(pred) / np.imag(pred).max()))
                nmse_array[i, j] = 0.5*(nmse(np.real(gt) / np.real(gt).max(), np.real(pred) / np.real(pred).max())+nmse(np.imag(gt) / np.imag(gt).max(), np.imag(pred) / np.imag(pred).max()))
    else:
        psnr_array = np.zeros((1, gt_recon.shape[-1]))
        ssim_array = np.zeros((1, gt_recon.shape[-1]))
        nmse_array = np.zeros((1, gt_recon.shape[-1]))
        ...
        for j in range(gt_recon.shape[-1]):
            pred, gt = pred_recon[:, :, j], gt_recon[:, :, j]
            psnr_array[0,j] = 0.5*(psnr(np.real(gt) / np.real(gt).max(), np.real(pred) / np.real(pred).max())+psnr(np.imag(gt) / np.imag(gt).max(), np.imag(pred) / np.imag(pred).max()))
            ssim_array[0,j] = 0.5*(ssim(np.real(gt) / np.real(gt).max(), np.real(pred) / np.real(pred).max())+ssim(np.imag(gt) / np.imag(gt).max(), np.imag(pred) / np.imag(pred).max()))
            nmse_array[0,j] = 0.5*(nmse(np.real(gt) / np.real(gt).max(), np.real(pred) / np.real(pred).max())+nmse(np.imag(gt) / np.imag(gt).max(), np.imag(pred) / np.imag(pred).max()))
